...
const DELTA = 0.1;
const formattingParamsToLoad =
  "text, font, alignment, lineSpacing, style, leftIndent, rightIndent, listItemOrNullObject, uniqueLocalId";
...
/**
 * Connector object for formatting operations.
 */
window.formattingConnector = {
  /**
   * Checks the formatting of the document.
   * @param {boolean} start - Indicates whether to start from the first paragraph or the selected paragraph.
   * @param {object} data - The data service object containing formatting information.
   * @returns {Promise<object>} - A promise that resolves to an object containing the formatting errors.
   */
  ...
  /**
   * Corrects the formatting errors in a paragraph.
   * @param {string} idToCorrect - The unique ID of the paragraph to correct.
   * @param {Array<string>} errors - The array of formatting errors to correct.
   * @returns {Promise<boolean>} - A promise that resolves to a boolean indicating whether the correction was successful.
   */
...
/**
 * Starts the formatting scan of the document.
 * @param {boolean} start - Indicates whether to start from the first paragraph or the selected paragraph.
 * @returns {Promise<object>} - A promise that resolves to an object containing the formatting errors.
 */
  ...
  return Word.run(async (context) => {
    ...
    while (paragraph !== null && !paragraph.isNullObject) {
      ...
      if (
        ...
        const styleChecks = [
          {
            condition: paragraph.font.name !== dataService.fontName,
            errorType: formattingErrorTypes.INCORRECT_FONT_NAME,
          },
          {
            condition: paragraph.font.size !== GetExpectedFontSize(paragraph),
            errorType: formattingErrorTypes.INCORRECT_FONT_SIZE,
          },
          {
            condition: paragraph.alignment !== dataService.alligment,
            errorType: formattingErrorTypes.INCORRECT_ALIGNMENT,
          },
          {
            condition:
              paragraph.lineSpacing !== dataService.lineSpacingInPoints,
            errorType: formattingErrorTypes.INCORRECT_LINE_SPACING,
          },
          {
            condition:
              paragraph.leftIndent > dataService.leftIndentInPoints + DELTA ||
              paragraph.leftIndent < dataService.leftIndentInPoints - DELTA,
            errorType: formattingErrorTypes.INCORRECT_LEFT_INDENT,
          },
          {
            condition:
              paragraph.rightIndent > dataService.rightIndentInPoints + DELTA ||
              paragraph.rightIndent < dataService.rightIndentInPoints - DELTA,
            errorType: formattingErrorTypes.INCORRECT_RIGHT_INDENT,
          },
        ];
        // Iterate over checks
